...
/**
 * Kernel interface for LineupComponent. This component creates an NBA starting
 * lineup with players assigned to certain positions.
 */
public interface LineupComponentKernel extends Standard<LineupComponent> {
     ...
     *
     * @return iterates over all positions
     * @ensures result = Point Guard, Shooting Guard, Small Forward, Power
     *          Forward, Center
     */
    Iterator<String> positions();

    /**
     * Returns an iterator of players at a given position.
     *
     * @param position
     *            the position to retrieve players from
     * @requires position is a valid lineup position
     * @return iterator over players at that position
     * @ensures result = [players at given position]
     */
    Iterator<PlayerBasics> playersAt(String position);
